...
def main():
    ...
    EPOCHS = 1
    MAX_LENGTH = 1024  # Keep same sequence length as Phase 2

    # Path to Phase 2 model
    PHASE2_MODEL_PATH = "./pyr-135m-base-2"  # Adjust if your Phase 2 output dir is different

    print(f"Dataset size: {DATASET_SIZE:,} samples")
    print(f"Training sequence length: {MAX_LENGTH}")
    print(f"Loading from Phase 2 model: {PHASE2_MODEL_PATH}")
    ...
    output_dir = f"./pyr-135m-base-3"

    batch_size = 8  # Same as Phase 2 for 1024 sequences
    grad_accum = 32  # Same effective batch size as Phase 2
    ...
    # Even lower learning rate for Phase 3 - fine-tuning on specialized data
    ...
    print("\n" + "=" * 70)
    print("STARTING PYR PHASE 3 PRETRAINING - PYTHON EDU")
    print("=" * 70)
    ...
    try:
        ...
        with open(f"{output_dir}/training_info.txt", "w") as f:
            f.write(f"Pyr 135m Base - Phase 3 (Python EDU)\n")
    ...
    print("\n" + "=" * 70)
    print("PYR PHASE 3 PRETRAINING COMPLETE")
    print("=" * 70)
    print("Ready for instruction tuning (Phase 4)!")
