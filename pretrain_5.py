...
def main():
    ...
    # Path to Phase 4 model
    PHASE4_MODEL_PATH = "./pyr-135m-base-4"

    print(f"Dataset size: {DATASET_SIZE:,} samples")
    print(f"Training sequence length: {MAX_LENGTH}")
    print(f"Loading from Phase 4 model: {PHASE4_MODEL_PATH}")

    print("\nLoading tokenizer from Phase 4...")
    tokenizer = AutoTokenizer.from_pretrained(PHASE4_MODEL_PATH, use_fast=True)

    print(f"Tokenizer loaded: vocab_size={tokenizer.vocab_size}")

    print(f"\nLoading Pyr model from Phase 4...")
    ...
    try:
        ...
        with open(f"{output_dir}/training_info.txt", "w") as f:
            ...
            f.write(f"Continued from: {PHASE4_MODEL_PATH}\n")
